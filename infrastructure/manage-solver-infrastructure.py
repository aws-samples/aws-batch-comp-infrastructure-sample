...
class ECR:
    ...
    def delete_repository_images(self, repository_name) -> None:
        try:
            images = self.ecr.list_images(repositoryName=repository_name)
            if images["imageIds"]:
                self.ecr.batch_delete_image(
                    repositoryName=repository_name, imageIds=images["imageIds"]
                )
        except self.ecr.exceptions.RepositoryNotFoundException as e:
            logger.info(
                f"Repository {repository_name} not found (already deleted).  Error: {e}"
            )
        except Exception as e:
...
class S3Filesystem:
    ...
    def upload_file_to_s3(
        self, bucket_name: str, file_name: str, object_name=None
    ) -> None:
...
class SSM:
    ...
    def get_ami_image(self) -> str:
        try:
            ...
            logger.error(
                f"Failed to call ssm.get_parameters to find the recommended AMI for linux: {e}"
            )
            raise e
...
class CloudFormation:
    ...
    def create_parameters(self, project_name, instance_type, ami_id, container_memory):
        return [
            ...
            {"ParameterKey": "AvailZoneId", "ParameterValue": "a"},
    ...
    def create_cloudformation_stack(
        self, project_name, instance_type, ami_id, container_memory
    ) -> None:
        try:
            ...
            response = self.cfn.create_stack(
                ...
                Parameters=self.create_parameters(
                    project_name, instance_type, ami_id, container_memory
                ),
            )
        except self.cfn.exceptions.AlreadyExistsException as exists:
            logger.error(
                "Stack already exists: perhaps it failed to properly create?  Try deleting it with delete-solver-infrastructure."
            )
            raise e
        except Exception as e:
    ...
    def await_completion(self, mode) -> bool:
        while True:
            ...
            try:
                ...
                logger.info(
                    f"Stack {mode} operation in progress.  Cloudformation states stack is deleted."
                )
                return mode == "delete"

            logger.info(
                f"Stack {mode} operation in progress.  Current status: {stack.stack_status}"
            )
            ...
            if "IN_PROGRESS" in stack.stack_status:
                ...
                time.sleep(30)
            elif "ROLLBACK" in stack.stack_status or "FAILED" in stack.stack_status:
                ...
                raise Exception(
                    f"Unexpected cloudformation state when awaiting completion: {stack.stack_status}"
                )
...
def arg_parser() -> dict:
    ...
    parser.add_argument(
        "--profile", required=False, help="AWS profile (uses 'default' if not provided)"
    )
    parser.add_argument(
        "--project",
        required=False,
        default="comp24",
        help="ADVANCED USERS ONLY: Name of the project (default: 'comp24').",
    )
    parser.add_argument(
        "--solver-type",
        required=True,
        choices=("cloud", "parallel"),
        help="Either 'cloud' or 'parallel' depending on the desired configuration.",
    )
    parser.add_argument(
        "--mode",
        required=False,
        ...
        help="One of 'create', 'update', 'delete': create, update or delete the infrastructure.",
    )
    ...
    if not re.match(r"^[a-z]([a-z\d\/_-])*$", args.project):
        ...
        logger.error(
            f"Project names must start with a lowercase letter and can consist only of lowercase numbers, digits, '/', '_', or '-' characters."
        )
        sys.exit(-1)
...
def main() -> None:
    ...
    try:
        ...
        logger.error(
            f"Unable to create AWS session.  Please check that default profile is set up in the ~/.aws/config file and has appropriate rights (or if --profile was provided, that this profile has appropriate rights)"
        )
        sys.exit(1)

    if not session.region_name:
        logger.error(
            f"Profile does not have a region defined.  Please add a region (recommend: us-east-1) to profile '{profile}' in the ~/.aws/config file"
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.error(
            "An error occurred during an AWS operation.  Usually this is caused by the AWS session having insufficient rights to resources.  Please double check that the default profile is properly set up in your AWS Config."
        )
